    ...
    public abstract class DbContext
    {
        ...
        public void SaveChanges()
        {
            ...
            foreach (IEnumerable<object> dbSet in dbSets)
            {
                ...
                using (new ConnectionManager(connection))
                {
                    ...
                    {
                        foreach (IEnumerable dbSetT in dbSets)
                        {
                            var dbSetType = dbSetT.GetType().GetGenericArguments().First();
                            ...
                            try
                            {
                                persistMethod.Invoke(this, new object[] { dbSetT });
                            }
            ...
            where TEntity : class, new()
        {
            ...
            foreach (var foreignKey in foreignKeys)
            {
                ...
                var navigationPropetry = entityType.GetProperty(navigationPropertyName);

                var navigationDbSet = this.dbSetProperties[navigationPropetry.PropertyType].GetValue(this);

                var navigationPrimaryKey = navigationPropetry.PropertyType.GetProperties().First(pi => pi.HasAttribute<KeyAttribute>());
                ...
                foreach (var entity in dbSet)
                {
                    ...
                    navigationPropetry.SetValue(entity, navigationPropertyValue);
                }
            }
        }
        ...
        {
            var dbSet = this.GetType().GetProperties().Where(pi => pi.PropertyType.GetGenericTypeDefinition() == typeof(DbSet<>)).ToDictionary(pi => pi.PropertyType.GetGenericArguments().First(), pi => pi);
